...
Imporved: 1.增加了网络的深度，从而保证感受野与Ours相同
        2.调整ngf，使得网络参数总数为56,855,358
        3.使用UNet结构，不使用图像自身重构
"""
...
class create_model(Model):
    ...
    def build_generator(self, im_batch, out_channels, is_training, scope, reuse):
        # encoder
        tmp = []
        with tf.variable_scope(scope[0]):
            if reuse[0] is True:
            ...
            for index, (channels, kernel, stride) in enumerate(layer_specs):
                ...
                out = self.conv(out, channels, kernel, stride, sn=self.sn,
                                scope=scope[0] + '_encoder%d_%d' % ((index % 2 + 1), (index / 2 + 1)))
                out = self.batchnorm(out, is_training,
                                     scope[0] + '_encoder%d_%d' % ((index % 2 + 1), (index / 2 + 1)))
                tmp.append(out)
        ...
        with tf.variable_scope(scope[1]):
            if reuse[1] is True:
                tf.get_variable_scope().reuse_variables()
            for i in range(3):
                out = self.conv(self.lrelu(out), ngf * 16, kernel=3, stride=1, sn=self.sn, scope=scope[1] + '_latent%d' % (i + 1))
                out = self.batchnorm(out, is_training, scope[1] + '_latent%d' % (i + 1))
        ...
        with tf.variable_scope(scope[2]):
            ...
            for index, (channels, kernel, stride) in enumerate(delayer_specs):
                ...
                out = self.deconv(out, channels, kernel, stride, sn=self.sn,
                                  scope=scope[2] + '_decoder%d_%d' % ((index % 2 + 1), (6 - index / 2)))
                out = self.batchnorm(out, is_training, scope[2] + '_decoder%d_%d' % (
    ...
    def compute_loss(self, im_batch):
        # im_batch[]: input_a, input_b, fake_a, fake_b, cyc_a, cyc_b, predict_fake_a, predict_fake_b,
        ...
        with tf.name_scope("generator_loss"):
            ...
            # cyclic reconstruct
            gen_loss_cyc = (loss.L1_loss(im_batch[0], im_batch[4]) + loss.L1_loss(im_batch[1], im_batch[5])) / 2.0
            # fake gan
            gen_loss_gan = (loss.gan_loss_generator(im_batch[6]) + loss.gan_loss_generator(im_batch[7])) / 2.0
            gen_loss_l1 = gen_loss_fake * 2. + gen_loss_cyc
            # generator loss
            gen_loss = gen_loss_gan * self.gan_weight + gen_loss_l1 * self.l1_weight

            losses = losses + [gen_loss_fake, gen_loss_cyc, gen_loss_gan, gen_loss_l1, gen_loss]

        with tf.name_scope("discriminator_loss"):
            discrim_loss_a = loss.gan_loss_discriminator(im_batch[8], im_batch[6])
            discrim_loss_b = loss.gan_loss_discriminator(im_batch[9], im_batch[7])
            losses = losses + [discrim_loss_a, discrim_loss_b]

        with tf.name_scope("generator_train"):
            gen_tvars = [var for var in tf.trainable_variables() if 'CycleGAN1' in var.name or 'CycleGAN2' in var.name
                         or 'Shared' in var.name]
    ...
    def train(self):
        ...
        with tf.device('/cpu:0'):
            ...
            for i in range(4):
                with tf.device('/gpu:%d' % (i)):
                    ...
                    with tf.variable_scope("generator", reuse=tf.AUTO_REUSE):
                        self.fake_ab = self.build_generator(_a, self.SAR_IMG_CHANNELS, self.is_training, ['CycleGAN1', 'Shared',
                                                    'CycleGAN2'], [False, False, False])
                        self.fake_ba = self.build_generator(_b, self.OPT_IMG_CHANNELS, self.is_training, ['CycleGAN2', 'Shared',
                                                    'CycleGAN1'], [False, True, False])
                        self.cyc_ba = self.build_generator(self.fake_ab, self.OPT_IMG_CHANNELS, self.is_training, ['CycleGAN2',
                                                    'Shared', 'CycleGAN1'], [True, True, True])
                        self.cyc_ab = self.build_generator(self.fake_ba, self.SAR_IMG_CHANNELS, self.is_training, ['CycleGAN1',
                                                    'Shared', 'CycleGAN2'], [True, True, True])
                    ...
                    with tf.variable_scope("discriminator", reuse=tf.AUTO_REUSE):
                        ...
                        im_batch = [_a, _b, self.fake_ba, self.fake_ab, self.cyc_ba, self.cyc_ab,
                                    self.predict_fake_a, self.predict_fake_b, self.predict_real_a, self.predict_real_b]
            ...
            with tf.device('/gpu: 0'):
                ...
                self.gen_loss_fake, self.gen_loss_cyc, self.gen_loss_gan, self.gen_loss_l1, self.gen_loss, \
            ...
            with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
                ...
                g_loss_cyc = np.zeros(self._save_every_iterations // 4, dtype=float)
                g_loss_gan = np.zeros(self._save_every_iterations // 4, dtype=float)
                g_loss_l1 = np.zeros(self._save_every_iterations // 4, dtype=float)
                ...
                for epoch in range(0, self._max_step):
                    ...
                    for i in range(0, self._save_every_iterations // 4):
                        ...
                        _, _, _, gen_loss_fake, gen_loss_cyc, gen_loss_gan, gen_loss_l1, gen_loss, \
                            discrim_loss_a, discrim_loss_b = sess.run([self.gen_train, self.discrim_train_a, self.discrim_train_b,
                            self.gen_loss_fake, self.gen_loss_cyc, self.gen_loss_gan, self.gen_loss_l1,
                            self.gen_loss, self.discrim_loss_a, self.discrim_loss_b], feed_dict={self.input_a: image_opt[0:4],
                        ...
                        g_loss_cyc[i] = gen_loss_cyc
                        g_loss_gan[i] = gen_loss_gan
                        g_loss_l1[i] = gen_loss_l1
                        g_loss[i] = gen_loss
                        d_loss_a[i] = discrim_loss_a
                        d_loss_b[i] = discrim_loss_b

                        print('gen_loss: {}, discrim_loss_a: {}, discrim_loss_b: {}, gen_loss_gan: {}, gen_loss_l1: {}, '
                              'gen_loss_fake: {}, gen_loss_cyc: {}'
                            .format(np.mean(g_loss[np.where(g_loss)]), np.mean(d_loss_a[np.where(d_loss_a)]),
                              ...
                              np.mean(g_loss_l1[np.where(g_loss_l1)]), np.mean(g_loss_fake[np.where(g_loss_fake)]),
                              np.mean(g_loss_cyc[np.where(g_loss_cyc)])))
                    ...
                    target.write("gen_loss_l1:\t%f\n" % np.mean(g_loss_l1[np.where(g_loss_l1)]))
                    target.write("gen_loss_fake:\t%f\n" % np.mean(g_loss_fake[np.where(g_loss_fake)]))
                    target.write("gen_loss_cyc:\t%f\n" % np.mean(g_loss_cyc[np.where(g_loss_cyc)]))
                    ...
                    # test
                    index = 15
                    data = np.zeros((index * 2, 4, self.IMG_HEIGHT, self.IMG_WIDTH, 3), dtype=np.float32)
                    ...
                    for i in range(0, index):
                        ...
                        fake_BA_temp, fake_AB_temp, cyc_BA_temp, cyc_AB_temp = sess.run([
                            self.fake_ba, self.fake_ab, self.cyc_ba, self.cyc_ab], feed_dict={
                        ...
                        data[i, 2, :, :, :] = (np.squeeze(cyc_BA_temp[0]) + 1) * 127.5
                        data[i, 3, :, :, :] = np.repeat(np.expand_dims((np.squeeze(cyc_AB_temp[0]) + 1) * 127.5, axis=2), 3, axis=2)
                    ...
                    for i in range(0, index):
                        ...
                        data[i + index, 2, :, :, :] = (np.squeeze(cyc_BA_temp[0]) + 1) * 127.5
                        data[i + index, 3, :, :, :] = np.repeat(np.expand_dims((np.squeeze(cyc_AB_temp[0]) + 1) * 127.5, axis=2), 3, axis=2)
                    ...
                    gen_test_loss_cyc = np.zeros(_num_test // 4, dtype=float)
                    for i in range(_num_test // 4):
                        ...
                        gen_loss, gen_loss_gan, gen_loss_fake, gen_loss_cyc = sess.run(
                            [self.gen_loss, self.gen_loss_gan, self.gen_loss_fake, self.gen_loss_cyc], feed_dict={
                                self.input_a: image_opt[0:4], self.input_b: image_pol[0:4], self.is_training: True})
                        gen_test_loss[i] = gen_loss
                        gen_test_loss_gan[i] = gen_loss_gan
                        gen_test_loss_fake[i] = gen_loss_fake
                        gen_test_loss_cyc[i] = gen_loss_cyc
                    ...
                    target.write("gen_loss_cyc:\t%f\n" % np.mean(gen_test_loss_cyc[np.where(gen_test_loss_cyc)]))
                    target.close()
